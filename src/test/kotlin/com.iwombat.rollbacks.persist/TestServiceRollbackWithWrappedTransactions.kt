...
class  TestServiceRollbackWithWrappedTransactions() {
    ...
    fun `should fail on race condition with version`() {
        ...
        // assign location to frozen object all should be version 1 - this should cause a race condition
        // and rollback every item in the list
        val entitiesToChange = frozenValueObjects.map{it.convertToEntity()}
        try {
            wrapper.assignWrapper("home", entitiesToChange)
        } catch (e : ObjectOptimisticLockingFailureException) {
            println("failed assignment with message --------> ${e.message}")
            // eat our version mismatch failures
        }

        val testCycle = service.findMotorcycle(testCaseId)
        assertTrue(testCycle.version == 1)
        assertTrue((rollbackCounter.count > 0), "Rollback never happened")
